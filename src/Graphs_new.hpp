...
public:
  ...
  bool init(int inStartX, int inStartY, int inSizeX, int inSizeY,
            float inValXMin = 0, float inValYMin = 0, float inValXMax = 0,
            float inValYMax = 0) {
  ...
  int m_graphSizeX;
  int m_graphSizeY;
  // TODO1: graph_new and Renderer are strongly connected, maybe they should be
  // merged, graph rendering should be done in the graph class
