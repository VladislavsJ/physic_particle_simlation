...
void BorderInfo::setAnyBorderFlag() { data |= ANY_BORDER_MASK; }

void BorderInfo::clearALL() { data = 0; }
...
void BorderInfo::display() const {
  ...
  if (isAnyBorderSet()) {
    ...
    if (isBorderSet(BORDER_TOP))
      std::cout << "Top ";
    if (isBorderSet(BORDER_BOTTOM))
      std::cout << "Bottom ";
    if (isBorderSet(BORDER_LEFT))
      std::cout << "Left ";
    if (isBorderSet(BORDER_RIGHT))
      std::cout << "Right ";
    std::cout << "\n";
  }
}
